% load destrieux 2009 parcellation
% Herve Lombaert, August 10th, 2013
%
% Just output an ordered list of parcellation between 1:76
%
function [P,names] = load_parcellation(filename)

    [vertices,label,colortable]=read_annotation(filename);
    

    %idx = 1:76;
    %nlabels = numel(unique(label));
    idx = 1:colortable.numEntries;
    n = numel(vertices);
    P = zeros(n,1);
    for i=1:n
        idx_parcel = colortable.table(:,5)==label(i);
        if ~isempty(idx_parcel(idx_parcel>0))
            P(i) = idx(idx_parcel);
        end
    end
    
    names = colortable.struct_names;
